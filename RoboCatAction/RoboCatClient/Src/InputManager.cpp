...
void InputManager::HandleInput( EInputAction inInputAction, int inKeyCode )
{
	...
	case 'k':
		...
		break;
	case 'l':
		UpdateDesireVariableFromKey( inInputAction, mCurrentState.mIsShootingHyperYarn );
		break;
  case 'g':
		UpdateDesireVariableFromKey( inInputAction, mCurrentState.mIsShootingBallOfYarn );
		break;
	}
