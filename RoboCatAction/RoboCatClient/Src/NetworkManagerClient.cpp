...
void NetworkManagerClient::SetRoboCatToId(RoboCat* inRoboCat, int inPlayerId)
{
  mPlayerIdToRoboCatMap[inPlayerId] = inRoboCat;
}

RoboCat* NetworkManagerClient::GetRoboCatFromId(int inPlayerId) const
{
  auto it = mPlayerIdToRoboCatMap.find(inPlayerId);
  if (it != mPlayerIdToRoboCatMap.end())
  {
    return it->second;
  }
